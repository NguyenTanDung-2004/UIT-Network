...
export default function ConnectSidebar() {
  ...
  const currentType = pathname.split("/").pop() || "all";
  ...
  // Reset filter khi chuyển tab khác People
  useEffect(() => {
    if (currentType !== "peoples") {
      resetPeopleFilters();
      // Xóa query params filter people khỏi URL khi chuyển tab khác
      router.push(pathname, { scroll: false });
    } else {
      // Khi vào lại tab People, có thể lấy params từ URL để set lại filter nếu muốn
      // Hoặc giữ nguyên filter hiện tại
    }
  }, [currentType, pathname, router]);

  // Hàm cập nhật URL params, giữ nguyên params hiện tại, merge với newParams
  const updateUrlParams = (
    ...
    // Merge newParams vào currentParams
    Object.entries(newParams).forEach(([key, value]) => {
      if (
        ...
        if (key === "sort" && value === true) currentParams.set(key, "recent");
        else if (key === "filter" && value === true)
          currentParams.set(key, "seen");
        else currentParams.set(key, "true");
      } else {
